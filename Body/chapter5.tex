...
Software defined radios have become more commonplace in recent years and this has generated a number of COTS solutions.  A COTS solution is often a lower cost solution due to the mass manufacturing that takes place.  This has drive the cost of many SDRs to under one thousand dollars while still having excellent performance characteristics.  The N200 SDR purchased for this research cost fifteen hundred dollars and the daughter-board cost one hundred and fifty dollars approximately.  Many other software defined radios however have come out on the market since then.  Ettus for example has some that are below one thousand dollars and the author has also obtained the HackRF One SDR that now sells for three hundred dollars.  

However, the SDR is not the only component of a radiometer.  One of the key components is the amplification of the noise so that it can be detected.  This is a component that is hard to remove from the system.  

Add more after I run a few experiments

\subsection{Weight and component size benefits}

Look into various weights and sizes.  Talk about how we can now remove many filters (can be done in software) to reduce size, weight and complexity.  Talk about 
...
In contrast, a square-law detector usually has a very large bandwidth, as much as one gigahertz, and we usually need to filter to the frequency band of interest.  