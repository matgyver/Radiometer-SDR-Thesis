...
The strategy with the new design of the RF front end was to keep it simple.  This would result in a RF front end that used fewer components and would contribute less noise overall to the system.  An additional reason for doing this was to also keep the cost low.  Fewer components would require fewer interconnects and cables needed to connect the various components.  
...
The result of the design is a 3 stage system that uses 2 bandpass filters to help maintain the signal to the band of interest.  The final design, which can be seen in Figure \ref{ISU_Rad}, uses 3 identical LNAs.  The LNAs selected provide approximately 20 dB of gain at 1400 MHz.  After losses are taken out, our system gain is approximately 52.5 dB.  This system gain is very close to the ISU radiometer which gives us results that are similar to the ISU radiometer.  

\subsection{Existing ISU RF Front End}

As mentioned before, the ISU Radiometer was used for a while to conduct experiments.  The problems with the ISU radiometer was not a design issue, instead we believe that certain hardware on the radiometer failed causing the erroneous data.  Therefore, the ISU radiometer was studied and some ideas from the ISU radiometer were taken into consideration when designing the new RF front end.
