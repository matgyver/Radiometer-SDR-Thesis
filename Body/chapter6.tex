...
This chapter will present the results obtained from the experiments outlined in Chapter \ref{ch:exp_design}.  We discuss the analysis of the results and what was learned from each experiment.  This chapter concludes with a trade off analysis between SDR-based radiometer in terms of price, weight, and value.

\section{Experiment I - Software Defined Radio Based Radiometer Verification and Calibration} \label{Exp1_results}
As outlined in Chapter \ref{ch:exp_design}, Section \ref{Exp1}, this experiment is to verify the operation of a software defined radio based radiometer.  This is done by performing experiments that are similar to the verification and calibration methods used for a traditional radiometer.  By comparing these results to a square-law detector, receiving the same signal, we can verify the operation of the software defined radio based radiometer.

\subsection{Data Collected}\label{Exp1_data}

The data collected for experiment one is the total power measurements collected from the software defined radio based radiometer and the square-law detector.  This data is then calibrated by using the known physical temperature of the matched load.  Table \ref{exp1_datapoints} shows the values collected during the experiment and is used to calibrate the raw total power measurements.
...
\isucaption{Graph of the calibrated noise temperature for Experiment I}
\label{SDR_Calibrated}
\end{figure}

\emph{Square Law Detector Data.}  Now that we have looked at the software defined radio data, we want to look at the square-law detector with the end result of comparing the two.  The square-law detector gives us power information as a voltage, so once again we will need to calibrate this to the known temperature references.  It can be see in Figure \ref{X2_Raw} that the data from the square-law detector is very noisy.  Therefore, we need filter the data to smooth out the data.

\begin{figure}[h!tb] \centering
\includegraphics[width=\textwidth]{Experiments/Exp1/noisy_voltage.pdf}
\isucaption{Unfiltered data from the square-law detector used in Experiment I}
\label{X2_Raw}
\end{figure}

We can use Python to process this information and specifically we will use SciPy which includes several useful signal processing modules.  For this experiment, we will use a low pass filter to smooth out the signal.  The following code applies the filter to the data.
...
\emph{SDR-based radiometer vs square-law detector.}  We now want to compare both the Software Defined Radio Based Radiometer and the square-law to make sure that they match up.  Because both the SDR-Based Radiometer and the square-law are now calibrated to a noise temperature, we can graph both sets of data and compare them to each other.
...
\emph{Sensitivity.}  Sensitivity for a radiometer is the $NE\Delta T$ that was covered in chapter \ref{ch:background} and is found in Equation \ref{NEAT_EQ}.  The $NE\Delta T$ is also the standard deviation of the data that we collect from the software defined radiometer.  
...
This gives us the result of 0.1 Kelvin for the expected $NE\Delta T$.  Since the sensitivity is related to the standard deviation of the graph, we can use Python to give us the standard deviation of the data graphed in Figure \ref{Sensitivity_graph}.  Figure \ref{Sensitivity_graph} uses the data graphed in \ref{SDR_Calibrated} but zoomed in from 350 to 425 seconds. 

\begin{figure}[h!tb] \centering
\includegraphics[width=\textwidth]{Experiments/Exp1/SDR_Zoom.pdf}
\isucaption{Graph of the calibrated total power from experiment one while submerged in LN2 between 350 and 425 seconds.}
...
The addition of an interfering signal is unwanted to the radiometer and has an adverse effect on how the radiometer operates[\cite{Ellingson}].  This is a growing problem for all radiometers, but is a greater issue with radiometer used in orbiting spacecrafts as they see larger areas of the Earth [\cite{DeRooRFI}].  Even though the band we are working in of 1.4 GHz is an internationally protected frequency, there has been both intentional and unintentional radiators detected in current space borne radiometers that cause interference[\cite{Forte}].  Other methods have been explored in RFI detection and mitigation in traditional radiometers [\cite{McIntyre_RFI}] [\cite{DeRoo}].
...
We begin by looking at the what happens to our total power readings with no filter applied.  As we stated in section \ref{exp3_setup}, the frequency of the offending signal will not change, but the amplitude will.  This will result in clear indications of the total power changing as the amplitude of the offending signal changes.  
...
It can be seen that there are pulses that occur in the graph in Figure \ref{sdr_unfilt_raw} and that changes in the amplitude of the offending signal affect our total power readings.  These same pulses can also be seen in the square-law detector data shown in Figure \ref{x2_unfilt}.
...
We can see in both the software defined radio and the square-law detector that there is an interfering signal.  If we now look at the spectrum view on the software defined radio, we can see the signal in questions which is at 1.405 GHz and shown in Figure \ref{spectrum_interfering.}  The square-law detector has no frequency information, so our only method to detect an interfering signal is by looking at the total power readings.  In Figure \ref{X2_Calibrated} we can see the spikes in the square-law data, however, we do not know where in the spectrum that signal is at.
...
Since we know where the offending signal is, we can now design a filter to remove this signal.  In the GNURadio program we can specify both the frequency and the bandwidth that we desire for this band-reject filter.  Ideally we want to keep the bandwidth of the filter as tight as possible to the offending signal but we also want to make sure our filter is effective in removing the signal.  Figure \ref{spectrum_filter} shows the spectrum display from the software defined radio with the filter now turned on and filtering the offending signal.

\begin{figure}[h!tb] \centering
\includegraphics[width=\textwidth]{Images/spectrum_filter.png}
\isucaption{Image showing the spectrum view from the SDR-based radiometer with the filter on and filtering the offending signal}
...
The goal of this experiment is to examine what affect adding a filter does to the performance of the radiometer.  While we have demonstrated that we can filter an offending signal, this comes at a cost.  This experiment examines this impact on the measurement of total power while filtering an interfering signal.

\subsection{Data Collected}
The data collected for this experiment is the data points collected in table \ref{exp4_data_power} and table \ref{exp4_datapoints}.  The remaining data is data generated from Equations \ref{NEAT_FILTER} and \ref{PWR_FILTER}.
...
Because we are notching out a portion of the bandwidth in order to remove the offending signal, this also takes away that bandwidth for total power detection.  As the bandwidth of the filter ($\beta_{filter}$) increases, the more bandwidth that is subtracted from the total bandwidth ($\beta$) available.  Equation \ref{NEAT_FILTER} accounts for this loss by subtracting the filter bandwidth ($\beta_{filter}$) from the total bandwidth ($\beta$).  
...
Figure \ref{neat_bw} shows measured standard deviation points for the software defined radiometer for set filter sizes.  These filter sizes and collected data is in table \ref{exp4_data_power}.  It can be seen in figure \ref{neat_bw} that there is a good correlation between the expected sensitivity of the radiometer and the measured sensitivity of the radiometer.
...
We now look at the relationship of the total power received as the bandwidth decreases.  Figure \ref{power_bw} shows us both the measured total power received and the expected total power received as the bandwidth of the filter increases.
...
While SDR technology has advanced, bandwidth is still a constraint that affects SDRs and in turn a SDR-based Radiometer.  Bandwidth plays a critical role in the radiometer's sensitivity as explained in this thesis, therefore the fact that many SDRs are limited in bandwidth does create a disadvantage.  In many cases this bottleneck takes place in both the transport and processing of large bandwidth systems.  This also relates to the power consumption disadvantage since larger bandwidth also means requiring additional computational cycles as well.  
