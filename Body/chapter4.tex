...
%To implement this low pass filter as a digital filter, a single pole Infinite Impulse Response (IIR) filter is used.  We can examine how a IIR filter is identical to a RC filter by looking at the step response of this filter as we did with the RC filter step response.
...
%An analog RC filter has a similar decay, referred to as the time constant ($\tau$), that is determined by multiplying the physical value of the resistor and capacitor used in the circuit shown in Figure \ref{rc_circuit} and is calculated in Equation \ref{RC_tc}.  This value is the number of seconds it takes for the circuit to decay to 36.8\% of its final value.  

%\begin{equation}\label{RC_tc}
%\tau = RC \ \ sec
%\end{equation}

%The time constant for a single pole low pass filter can be found in a similar way, where we want to find how many samples ($d$) it will take for our recursive filter to decay to 36.8\% of its final value.  Equation \ref{single_pole_tc} shows us the relationship between the number of samples, $d$, to $x$ which is our decay between samples.

%\begin{equation}\label{single_pole_tc}
%x = e^{\frac{-1}{d}}
%\end{equation}

%Equation relates this time constant to our value of $x$ for a desired cutoff frequency of $f_c$.  

%\begin{equation}\label{single_pole_fc}
%x = e^{-2 \pi f_{c}}
%\end{equation}

%Figure \ref{rc_response} shows that both the RC filter and the IIR filter respond identically to the same input, in this case a step function.  We can determine our coefficients of our single pole recursive filter by looking at the number of samples it takes to decay to 36.8\% of the final value as you do with an analog RC filter.  Therefore our single pole IIR filter functions identically to a RC low pass filter.  
