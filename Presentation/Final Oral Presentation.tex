...
\begin{itemize}
\item Radiometers are used in a variety of remote sensing applications such as soil moisture and ocean salinity
...
The Shirleys Bay Radio Astronomy Consortium (SBRAC) located in Smiths Falls, Ontario is currently using a USRP software defined radio in conjunction with GNURadio\cite{Leech2007}.  

\begin{itemize}
\item Tuned to the hydrogen line at 1420.4058 MHz
\item Marcus Leech is the Principal Investigator
\item Mr. Leech has made contributions to GNURadio related to radiometry
\end{itemize}
...
SBRAC has successfully used this configuration to obtain radio astronomy data by looking at the hydrogen line at 1420.4058 MHz.  The person in charge of this facility, Marcus Leech, contributed software to the GNURadio specifically for radio astronomy applications.  It was this software branch that was used as the base for the GNURadio program that was used in this thesis.}
\end{frame}
...
\begin{itemize}
\item Total system noise
\item Bandwidth of the signal
...
\note{To measure power we begin with the noise signal coming from the antenna.  Our antenna is assumed to be looking at our target of interest and it is assumed that we can relate the antenna noise to the noise from the source.  It is often easier to refer to this noise as the brightness temperature.  Therefore the brightness temperature of the source can be related to the brightness temperature at the antenna.  We will refer to this brightness temperature as T$_{A}$}
\end{frame}

\begin{frame}
\frametitle{General Theory}
\begin{block}{Measuring power}
A more realistic model is a system that includes the noise that is generated by the radiometer itself.  This noise is added to noise from the antenna.  
\end{block}
{\begin{figure}[h!tb] 
\centering
\includegraphics[width=\textwidth]{../Images/radiometer_noise_added.png}
\label{rad_noise_added}
\end{figure}
}
\note{The antenna itself also adds noise.  Our greatest contributor to noise is from the LNAs.  However mismatches in the RF chain and other factors can also increase this noise.  This noise covers up or obscures the noise that we are interested in so we want to keep this to a minimum.}
\end{frame}
...
The RF front end is one of the most critical components in a radiometer.  Although we have moved most of the functionality in to software with the SDR, we still need to amplify the signal.  This is where Low Noise Amplifiers (LNAs) come in to play, they amplify the signal while keeping the noise they contribute as low as possible.  


\begin{figure}\label{AD_RFSIM}
\includegraphics[width=0.6\linewidth]{images/RF_Front_end.png}
\end{figure}
\note{The image shown is Analog Devices program for analyzing gain, noise temperature and losses in a RF chain.  This program was used to analyze ours and other RF Front end systems.  The system shown is a simplified version of the RF front end used in testing.  Although filters were included in this image, a SDR does not require filters as we can filter in the SDR itself.

While LNAs attempt to keep the noise they add as low as possible, they are usually still our largest contributor to the total system noise that we see.  However LNAs are often a vast improvment over other amplifiers that may be used in more general communication equipment.
}
\end{frame}
...
\begin{frame}
\frametitle{Software Defined Radiometer}
...
\begin{itemize}
\item Filter to the bandwidth we desire
\item Total power measurement
\item Integrate 
\item Record the data
\end{itemize}

\note{Filtering can be done by limiting the sampling rate of the SDR, but we can also add additional filtering if desired.  } 
\end{frame}

\begin{frame}
\frametitle{Software Defined Radiometer}
Total power measurements is accomplished by squaring the the phase and magnitude information provided by the SDR.  Integration is a simple mathematical operation on the signal.  Recording the data is also trivial for the program to accomplish.

{\begin{figure}[h!tb] 
\centering
\includegraphics[width=\textwidth]{../Images/TPR_grc.png}
\label{simplerad}
\end{figure}
}

\note{The block shown is the TPR block we used in GNURadio.  This combines the total power measurement and integration and low-pass filter into one block that we can call in other programs.} 
\end{frame}
%---------------------------------
...
Experimental data was collected to verify the operation of software defined radio as a radiometer.  In addition, we performed an experiment that demonstrated an additional benefit of using a SDR over a more traditional radiometer.
\begin{figure}\label{exp_setup}
\includegraphics[width=.6\linewidth]{../Images/exp1_setup.jpg}
\end{figure} 
\end{frame}

\subsection{Experiment 1 - Normal Operation}
\begin{frame}
\frametitle{Normal operation}

Experiment 1 is designed to compare the operation of the SDR radiometer and compare it to a traditional radiometer using a square-law detector.  This was accomplished by using a common RF front end and splitting the signal to both the SDR and the square-law detector.
\begin{figure}\label{rq_annotate}
\includegraphics[width=1.0\linewidth]{../Experiments/Exp1/rqvstime_annotate.pdf}
\end{figure} 
\end{frame}
...
Finally, we can do a hypothetical were we calibrate the radiometer to say that LN2 is "dry soil" and our ice water bath is our "wet soil".  We can now interpolate the data to a soil moisture percentage.
\begin{figure}\label{calib_soilmoisture}
\includegraphics[width=1.0\linewidth]{../Experiments/Exp1/sdr_soilmoisture.pdf}
\end{figure} 
\end{frame}

\subsection{Experiment 2 - Interfering Signal}
\begin{frame}
\frametitle{Normal operation}

Experiment 2 is designed to again compare the SDR radiometer to a traditional radiometer but now have an unwanted signal present.  The SDR is able to filter this signal out, where the square-law detector is not able to do so.
...
\includegraphics[width=1.0\linewidth]{../Experiments/Exp2/sdr_calibrated_zoom.pdf}
\end{figure} 
\end{frame}
